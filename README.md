<p align="center">
  <img src="https://github.com/Zanooda/storoslop/blob/main/assets/hero.png?raw=true" alt="storoslop">
</p>

<p align="center">
  <strong>A coding agent with the IDE wired in.</strong>
  <strong><a href="https://github.com/Zanooda/storoslop">storoslop</a></strong>
</p>

<p align="center">
  <a href="https://www.npmjs.com/package/@oh-my-pi/pi-coding-agent"><img src="https://img.shields.io/npm/v/@oh-my-pi/pi-coding-agent?style=flat&colorA=222222&colorB=CB3837" alt="npm version"></a>
  <a href="https://github.com/Zanooda/storoslop/blob/main/packages/coding-agent/CHANGELOG.md"><img src="https://img.shields.io/badge/changelog-keep-E05735?style=flat&colorA=222222" alt="Changelog"></a>
  <a href="https://github.com/Zanooda/storoslop/actions"><img src="https://img.shields.io/github/actions/workflow/status/Zanooda/storoslop/ci.yml?style=flat&colorA=222222&colorB=3FB950" alt="CI"></a>
  <a href="https://github.com/Zanooda/storoslop/blob/main/LICENSE"><img src="https://img.shields.io/github/license/Zanooda/storoslop?style=flat&colorA=222222&colorB=58A6FF" alt="License"></a>
...
```sh
curl -fsSL https://raw.githubusercontent.com/Zanooda/storoslop/main/scripts/install.sh | sh
```
...
nix run github:Zanooda/storoslop

# Or install into the active profile
nix profile install github:Zanooda/storoslop
```

Flake consumers can use `packages.<system>.omp`, `overlays.default`, `nixosModules.default`, or `homeManagerModules.default`. A Home Manager configuration can install storoslop and own its settings declaratively:

```nix
{
  inputs.omp.url = "github:Zanooda/storoslop";
...
```powershell
irm https://raw.githubusercontent.com/Zanooda/storoslop/main/scripts/install.ps1 | iex
```

**Pinned versions (mise)**

```sh
mise use -g github:Zanooda/storoslop
```
...
`storoslop` generates its own completion scripts for **bash**, **zsh**, and **fish** from the live command/flag metadata, so they never drift from the actual CLI. Subcommands, flags, and enum values complete statically; model names (`--model`, `--smol`, `--slow`, `--plan`) resolve against the bundled model catalog and `--resume` against your on-disk sessions.

```sh
# zsh — add to ~/.zshrc (or write the output into a file on your $fpath)
eval "$(storoslop completions zsh)"

# bash — add to ~/.bashrc
eval "$(storoslop completions bash)"

# fish
storoslop completions fish > ~/.config/fish/completions/storoslop.fish
```

## Every tool, _benchmaxxed_.

Edits that land on the first attempt. Reads that summarize files instead of dumping their content. Searches that return instantly. Pick any model — storoslop will get it right.
...
Originally built on [Mario Zechner](https://github.com/mariozechner)'s wonderful [Pi](https://github.com/badlogic/pi-mono), storoslop adds everything you're missing.
...
![storoslop TUI: a single eval session with `[1/2] pandas describe` (Python) printing a real DataFrame.describe() table, followed by `[2/2] top scorer` (JavaScript) running a reduce. Footer: 'Both kernels ran in one session.'](https://omp.sh/captures/eval.webp)

### 02 · LSP wired into every write

Ask for a rename and you get a rename. The call goes through workspace/willRenameFiles, so re-exports, barrel files, and aliased imports update before the file moves. Everything your IDE knows, the agent knows.

![storoslop TUI: `LSP references` returns five hits across three files for the symbol `formatBytes`, then `LSP rename` applies the change with edits to format.ts/report.ts/cli.ts, then a `Search formatBytes 0 matches` confirmation. Final line: 'Rename complete. Five edits across three files…'.](https://omp.sh/captures/lsp.webp)
...
![storoslop TUI: /advisor status shows the advisor running on openai-codex/gpt-5.5; after the main agent scopes a catch to ENOENT instead of swallowing every error, an amber 'Advisor 1 note (concern)' card warns the fix no longer matches the user's literal acceptance criterion.](https://omp.sh/clips/advisor-poster.webp)

_[Watch the capture ↗](https://omp.sh/clips/advisor.mp4)_

### 07 · Hand someone the link, they're in.

/collab puts your live session on a relay and hands back a link — and a QR. A teammate joins from another terminal with storoslop join, or just opens it in a browser. Share read-write to pair on the same agent, or /collab view for a read-only link anyone can watch but no one can steer. Frames are sealed client-side; the relay never sees your keys.

![storoslop TUI: /collab view prints 'Collab session started!' with an storoslop join command, a my.omp.sh browser link, the note 'Anyone with this link can watch the session but cannot prompt the agent', and a large scannable QR code.](https://omp.sh/clips/collab-poster.webp)
...
![storoslop TUI: web_search returns 10 ranked Perplexity sources for inference-time compute scaling, the agent picks an arxiv paper, calls read https://arxiv.org/pdf/2604.10739v1, and summarizes the paper's headline result with real numbers.](https://omp.sh/clips/web-poster.webp)

_[Watch the capture ↗](https://omp.sh/clips/web.mp4)_

### 09 · Unapologetically native. Even on Windows.

Other agents shell out to rg, grep, find, and bash. On many machines those binaries don't exist, and on the ones where they do, every call costs a fork-exec round-trip. storoslop links the real implementations into the process. ripgrep, glob, find: in-process. brush is the bash — with sessions that survive across calls, and 58 command-line utilities (ls, sed, sort, xargs, even jq) ported into the builtins crate and run in-process, zero fork/exec. The same storoslop binary runs on macOS, Linux, and Windows — no WSL bridge.
...
![storoslop TUI reading pr://storoslop/1063 and then /diff/1, showing hunk headers, added lines, and a [MODIFIED] (+12 -0) summary.](https://omp.sh/captures/pr.webp)
...
![storoslop TUI: ✓ Read src/session.ts (⚠ 1 conflict), then ✓ Write conflict://1 · 1 line with content @theirs, then a confirmation 'Resolved.'](https://omp.sh/clips/conflict-poster.webp)
...
![storoslop TUI: ✓ AST Edit: console.log($X) (proposed) 3 replacements · 1 file, then ✓ Accept: 3 replacements in 1 file (AST Edit), followed by 'Applied 3 replacements in src/auth.ts.'](https://omp.sh/clips/codemod-poster.webp)
...
![storoslop TUI driving the browser tool against DuckDuckGo](https://omp.sh/captures/browser.webp)
...
[Full reference →](https://github.com/Zanooda/storoslop/docs/tools)
...
Define custom providers in `~/.storoslop/agent/models.yml`:
...
Run `storoslop models spark` to verify discovery. Then run `storoslop setup` and choose the model in the default-model step, or open `/model` in a session and assign it to the `default` role.

To preconfigure the default without the picker, add the selector to `~/.storoslop/agent/config.yml`:
...
Full provider & routing reference at [storoslop/docs/providers](https://github.com/Zanooda/storoslop/docs/providers).
...
[`web_search` reference ↗](https://github.com/Zanooda/storoslop/docs/tools#web_search)
...
Same engine, four wrappers. `storoslop` runs the TUI. `storoslop -p` answers a single prompt and exits. The Node SDK embeds the session in your process. `storoslop --mode rpc` and `storoslop acp` hand the wheel to another program over stdio.
...
![storoslop TUI: the ask tool renders an option picker with three choices, a (Recommended) badge on the first, and 'up/down navigate · enter select · esc cancel' footer.](https://omp.sh/captures/ask.webp)
...
Full reference: [storoslop/docs/sdk](https://github.com/Zanooda/storoslop/docs/sdk).

## A harness worth keeping is one you _don't_ outgrow.

Pick it up at **[storoslop](https://github.com/Zanooda/storoslop)**.

storoslop is a fork of [Pi](https://github.com/badlogic/pi-mono) by [Mario Zechner](https://github.com/mariozechner), rewritten as a coding-first surface: sessions, subagents, slash commands, extensions — all TypeScript, all MIT, all on [GitHub](https://github.com/Zanooda/storoslop). Shape it from config, hook it from outside, or read the source when you need to.
...
On first run storoslop inherits whatever is already on disk: rules, skills, and MCP servers from `.claude`, `.cursor`, `.windsurf`, `.gemini`, `.codex`, `.cline`, `.github/copilot`, and `.vscode`. No migration script.

### Extensibility

Ask storoslop to write the piece you're missing, then `/reload-plugins`. Keep it local, ship it in a `marketplace`, or publish it to npm.

## Philosophy

storoslop is a fork of [pi-mono](https://github.com/badlogic/pi-mono) by [Mario Zechner](https://github.com/mariozechner), extended with a batteries-included coding workflow.
...
storoslop is licensed under the [MIT License](LICENSE).
...
- [storoslop](https://github.com/Zanooda/storoslop)
- [GitHub](https://github.com/Zanooda/storoslop)
- [Changelog](https://github.com/Zanooda/storoslop/blob/main/packages/coding-agent/CHANGELOG.md)
- [npm](https://www.npmjs.com/package/@oh-my-pi/pi-coding-agent)
- [Discord](https://discord.gg/4NMW9cdXZa)
- [MIT](https://github.com/Zanooda/storoslop/blob/main/LICENSE)
