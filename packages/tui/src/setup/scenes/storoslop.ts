 ...
 * roster are bundled (see `config/storoslop-provider.ts` in pi-coding-agent);
 * the models config persists only the credential, so model updates ship with
 * the build.
 */

/** Bundled storoslop provider identity (mirrors pi-coding-agent's `config/storoslop-provider.ts`). */
