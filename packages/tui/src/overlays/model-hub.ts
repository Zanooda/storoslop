...
export class ModelHubComponent implements Component {
	...
	#reloadRoles(autoCandidates: ReadonlyArray<Model>): void {
		const allModels =
			this.#scopedModels.length > 0
				? autoCandidates
				: this.#registry.getAll("all").filter(m => m.provider === "storoslop");
	...
	#syncFromRegistryState(): void {
		...
		if (this.#scopedModels.length > 0) {
			...
			allModels = this.#registry.getAll("all").filter(m => m.provider === "storoslop");
			try {
	...
	#renderLockedView(entry: SidebarEntry, width: number, rows: number): string[] {
		...
		if (catalogCount > 0) {
			...
			const preview =
				this.#scopedModels.length > 0 ? [] : this.#registry.getAll("all").filter(m => m.provider === "storoslop");
