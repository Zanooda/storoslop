...
describe("profile directories", () => {
	...
	beforeEach(async () => {
		...
		configDir = `\.storoslop-profile-test-${Snowflake.next()}`;
	...
	it("keeps XDG-backed named profile state under profile-specific roots", async () => {
		...
		await fs.mkdir(path.join(process.env.XDG_DATA_HOME, "storoslop", "profiles", "work"), { recursive: true });
		await fs.mkdir(path.join(process.env.XDG_STATE_HOME, "storoslop", "profiles", "work"), { recursive: true });
		await fs.mkdir(path.join(process.env.XDG_CACHE_HOME, "storoslop", "profiles", "work"), { recursive: true });

		setProfile("work");

		expect(getAgentDbPath()).toBe(path.join(process.env.XDG_DATA_HOME, "storoslop", "profiles", "work", "agent.db"));
		expect(getSessionsDir()).toBe(path.join(process.env.XDG_DATA_HOME, "storoslop", "profiles", "work", "sessions"));
		expect(getPythonGatewayDir()).toBe(
			path.join(process.env.XDG_STATE_HOME, "storoslop", "profiles", "work", "python-gateway"),
		);
	});
	...
	it("does not silently switch a named profile to XDG once the base app dir appears", async () => {
		...
		await fs.mkdir(path.join(process.env.XDG_DATA_HOME, "storoslop"), { recursive: true });
		await fs.mkdir(path.join(process.env.XDG_STATE_HOME, "storoslop"), { recursive: true });
...
describe("dirs module import behavior", () => {
	...
	it("ignores inherited profile agent dir when OMP_PROFILE explicitly selects default", async () => {
		...
		const probeConfigDir = `\.storoslop-default-profile-${Snowflake.next()}`;
		try {
			const dirsUrl = url.pathToFileURL(path.join(import.meta.dir, "..", "src", "dirs.ts")).href;
	...
	it("honors XDG dir keys from a profile .env applied after the resolver froze", async () => {
		...
		const profileConfigDir = `\.storoslop-env-xdg-${Snowflake.next()}`;
		try {
			...
			// Named profiles only adopt XDG when their own XDG path already exists.
			const xdgProfileRoot = path.join(xdgStateRoot, "storoslop", "profiles", "work");
