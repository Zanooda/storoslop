...
bundles. Bun's `fetch` does not consume the env var natively, so storoslop injects
...
```bash
storoslop auth-broker login              # interactive provider selection
storoslop auth-broker login anthropic    # login to a specific provider
storoslop auth-broker login vllm         # store vLLM API key (or placeholder for local no-auth)
storoslop auth-broker list               # list supported providers
storoslop auth-broker logout             # interactive — pick a stored credential to remove
```
