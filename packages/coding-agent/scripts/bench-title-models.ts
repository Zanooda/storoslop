 ...
 *   bun scripts/bench-title-models.ts --db ~/.storoslop/agent/history.db --out bench.json
 */
...
function parseArgs(argv: string[]): BenchConfig {
	...
	return {
		dbPath: (get("--db") ?? path.join(os.homedir(), ".storoslop/agent/history.db")).replace(/^~/, os.homedir()),
