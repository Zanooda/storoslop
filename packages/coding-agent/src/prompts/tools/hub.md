...
Project-scoped long-running processes shared by every storoslop instance in the same directory. A long-running service, watcher, debugger, REPL, or process needing later input MUST use `op:"start"`, not `bash`.
...
- **`start`** launches `application` + `args` directly. `cwd` defaults to the session directory; `pty` defaults true.
  ...
  - `persist: true` opts out of last-storoslop teardown; `detached: true` survives broker shutdown and all storoslop exits (implies persist, disables PTY input). Omit both unless their survival guarantees are required.
