...
- `app.relay: true` → drive the user's own Chrome tabs via the storoslop browser relay (auto-started; needs the storoslop Browser Relay extension installed). `app.target` picks a tab by URL/title substring; without it the visible tab is adopted — and an `open` carrying `url` NAVIGATES that adopted tab.
