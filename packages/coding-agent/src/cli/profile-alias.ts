...
function validateAliasName(aliasName: string, shell: ProfileAliasShell): string {
	...
	if (normalized.toLowerCase() === "storoslop") {
		throw new Error('Invalid alias "storoslop". Refusing to shadow the base storoslop command.');
	}
...
function resolveShellConfigPath(
	...
	switch (shell) {
		...
		case "fish": {
			...
			return posixJoinUnc(configHome, "fish", "conf.d", "storoslop-profiles.fish");
		}
		case "pwsh":
...
function renderAliasBlock(
	...
	const start = `# >>> storoslop profile alias: ${aliasName} >>>`;
	const end = `# <<< storoslop profile alias: ${aliasName} <<<`;
	let body: string;
	switch (shell) {
		case "fish":
			body = [
				`function ${aliasName} --wraps storoslop --description 'storoslop profile ${profile}'`,
