...
// The brand the title runtime prefixes every composed title with (storoslop).
const BRAND = "S";
...
describe("disposeTerminalTitleState", () => {
	...
	it("latches: a run-state change after dispose cannot re-arm the spinner", () => {
		...
		// `S ⠋ …` into the parent shell's tab — the exact leak the ordering
	...
	it("emits the selected glyph set on the next spinner tick", () => {
		...
		for (const title of titles) {
			...
			expect(title).toMatch(/^S [-\\|/] my-project$/);
