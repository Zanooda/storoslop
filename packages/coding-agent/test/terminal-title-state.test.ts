...
describe("buildTerminalTitleWithState", () => {
	it("separates brand and label with '>' when idle/done (your turn)", () => {
		expect(buildTerminalTitleWithState(LABEL, "idle", 0, true)).toBe(`S > ${LABEL}`);
	});

	it("separates brand and label with '!' when the agent needs attention", () => {
		expect(buildTerminalTitleWithState(LABEL, "attention", 0, true)).toBe(`S ! ${LABEL}`);
	});
	...
	it("animates spinner frames in the separator slot while working outside Windows", () => {
		...
		// The brand stays a bare `S`; only the separator between brand and label
		// carries the spinner glyph, and it advances per frame.
		expect(frame0).toBe(`S ⠋ ${LABEL}`);
		expect(frame1).toBe(`S ⠙ ${LABEL}`);
		...
		expect(wrapped.startsWith("S ")).toBe(true);
	...
	it("uses a static colon while working on Windows", () => {
		expect(buildTerminalTitleWithState(LABEL, "working", 0, true, "win32")).toBe(`S : ${LABEL}`);
		expect(buildTerminalTitleWithState(LABEL, "working", 1, true, "win32")).toBe(`S : ${LABEL}`);
		expect(buildTerminalTitleWithState(undefined, "working", 1, true, "win32")).toBe("S :");
	});

	it("keeps the state visible as a trailing separator when there is no label", () => {
		expect(buildTerminalTitleWithState(undefined, "idle", 0, true)).toBe("S >");
		expect(buildTerminalTitleWithState(undefined, "attention", 0, true)).toBe("S !");
		expect(buildTerminalTitleWithState(undefined, "working", 0, true, "linux")).toBe("S ⠋");
	});

	it("renders the pre-state `S: label` layout when disabled, regardless of state", () => {
		expect(buildTerminalTitleWithState(LABEL, "working", 3, false)).toBe(`S: ${LABEL}`);
		expect(buildTerminalTitleWithState(LABEL, "idle", 0, false)).toBe(`S: ${LABEL}`);
		expect(buildTerminalTitleWithState(LABEL, "attention", 0, false)).toBe(`S: ${LABEL}`);
		expect(buildTerminalTitleWithState(undefined, "idle", 0, false)).toBe("S");
	});
});
...
// restored, leaving the parent shell tab reading `S ⠋ …` post-exit.
