 ...
 *
 * Both OMP-native loaders are covered: `.storoslop/mcp.json` (native provider) and a
...
test("requestIdFormat from .storoslop/mcp.json reaches the transport config", async () => {
	const configs = await loadFrom(path.join(".storoslop", "mcp.json"), {
...
test("instructions does not split one endpoint into two connections", async () => {
	const configs = await loadFrom(".storoslop/mcp.json", {
