 ...
 * `storoslop plugin list`). #4845 extended the same footgun to the multi-word
 ...
 * `storoslop plugin <action>` command rather than leaking to the model — while
...
describe("documented-but-unregistered plugin verbs do not leak to launch (#2935)", () => {
	test("bare `omp list` hints at `storoslop plugin list` instead of launching with 'list' as the prompt", () => {
		...
		expect("error" in result && result.error).toContain("storoslop plugin list");
	});

	test("bare `omp remove` hints at `storoslop plugin uninstall` instead of launching with 'remove' as the prompt", () => {
		const result = resolveCliArgv(["remove"]);
		expect(result).not.toEqual({ argv: ["launch", "remove"] });
		expect(result).not.toHaveProperty("argv");
		expect(result).toHaveProperty("error");
		expect("error" in result && result.error).toContain("storoslop plugin uninstall");
	});
	...
	test("multi-word `omp marketplace add xyz` hints at `storoslop plugin marketplace` instead of leaking to the prompt (#4845)", () => {
		const result = resolveCliArgv(["marketplace", "add", "xyz"]);
		expect(result).not.toEqual({ argv: ["launch", "marketplace", "add", "xyz"] });
		expect(result).not.toHaveProperty("argv");
		expect(result).toHaveProperty("error");
		expect("error" in result && result.error).toContain("storoslop plugin marketplace");
	});

	test("bare marketplace-family verbs hint at their `storoslop plugin` command (#4845)", () => {
		for (const [verb, hint] of [
			["marketplace", "storoslop plugin marketplace"],
			["discover", "storoslop plugin discover"],
			["upgrade", "storoslop plugin upgrade"],
			["uninstall", "storoslop plugin uninstall"],
			["enable", "storoslop plugin enable"],
			["disable", "storoslop plugin disable"],
		] as const) {
