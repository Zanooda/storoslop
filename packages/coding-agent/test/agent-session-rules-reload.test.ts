...
async function expectStickyRuleReload(
	...
	// Project scope: nearest `.storoslop/RULES.md` walking up from cwd.
	if (opts.scope === "user") setAgentDir(tempDir.path());
	const rulesMd =
		opts.scope === "user"
			? path.join(tempDir.path(), "RULES.md")
			: path.join(tempDir.path(), ".storoslop", "RULES.md");
	if (opts.seedInitial) {
...
describe("AgentSession session-local rule snapshot reload on session reset", () => {
	...
	it("resolves rule://<name> for a rulebook rule created after startup once the context resets", async () => {
		...
		// Empty `.storoslop/rules/` keeps the project config scope present without any rulebook rule yet.
		const rulesDir = path.join(tempDir.path(), ".storoslop", "rules");
