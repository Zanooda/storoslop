...
describe("resolvePlanAutosaveDir", () => {
	it("defaults to <project>/.storoslop/plans when unset", () => {
		const cwd = makeCwd();
		const settings = Settings.isolated();
		expect(resolvePlanAutosaveDir(settings, cwd)).toBe(path.join(cwd, ".storoslop", "plans"));
		expect(defaultPlanAutosaveDir(cwd)).toBe(path.join(cwd, ".storoslop", "plans"));
	});
...
describe("autosaveApprovedPlan", () => {
	it("writes nothing when autosave is disabled", async () => {
		...
		expect(await Bun.file(path.join(cwd, ".storoslop", "plans", "AUTH_PLAN.md")).exists()).toBe(false);
	});
	...
	it("saves the approved plan under the default dir", async () => {
		...
		expect(result).toBe(path.join(cwd, ".storoslop", "plans", "AUTH_STORAGE_PLAN.md"));
	...
	it("suffices colliding filenames instead of overwriting", async () => {
		...
		expect(first).toBe(path.join(cwd, ".storoslop", "plans", "AUTH_PLAN.md"));
		expect(second).toBe(path.join(cwd, ".storoslop", "plans", "AUTH_PLAN-1.md"));
...
describe("plan-yolo approval autosave", () => {
	...
	it("autosaves the approved plan and preserves the plan-yolo transition", async () => {
		...
		expect(await Bun.file(path.join(cwd, ".storoslop", "plans", "AUTH_PLAN.md")).text()).toBe("# Plan\n\nYolo.\n");
