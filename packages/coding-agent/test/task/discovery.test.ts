...
async function writeOmpPluginAgent(home: string): Promise<void> {
	const userPluginsRoot = path.join(home, ".storoslop", "plugins");
...
describe("discoverAgents", () => {
	...
	test("loads OMP agents but skips Claude Code custom agents", async () => {
		await fs.mkdir(path.join(projectDir, ".storoslop", "agents"), { recursive: true });
		await fs.writeFile(path.join(projectDir, ".storoslop", "agents", "omp-test-agent.md"), OMP_AGENT_MD);
		...
		expect(projectAgentsDir).toBe(path.join(projectDir, ".storoslop", "agents"));
	});

	test("loads agents from OMP npm plugins under <home>/.storoslop/plugins/node_modules", async () => {
	...
	test("CLI extension agents win over project `extensions:` settings on dedup", async () => {
		...
		await fs.mkdir(path.join(projectDir, ".storoslop"), { recursive: true });
		await fs.writeFile(
			path.join(projectDir, ".storoslop", "settings.json"),
