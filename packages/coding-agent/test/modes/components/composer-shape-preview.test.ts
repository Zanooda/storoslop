...
describe("composer shape preview", () => {
	...
	it("borrows status rows from the live status source per shape layout", async () => {
		...
		expect(box).toContain("storoslop"); // stand-in title forwarded to the status source
		...
		expect(band).toContain("storoslop");
		...
		expect(claude).toContain("storoslop");
		...
		expect(borderless).toContain("storoslop");
