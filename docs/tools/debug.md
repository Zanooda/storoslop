...
- **Custom adapter config**
  ...
  - Search order mirrors LSP config: project root, project config dirs (`.storoslop/`, `.claude/`, `.codex/`, `.gemini/`), user config dirs (`~/.storoslop/agent/`, `~/.claude/`, `~/.codex/`, `~/.gemini/`), plugin roots, then home-root fallback. Files are merged from lowest to highest priority.
...
- The adapter runs under `node` if on `PATH`, otherwise under the storoslop host (Bun); `resolveDefaultJsDebugAdapter()` falls back to `process.execPath`, so a Bun-only setup is supported.
