...
- **Headless:** creates a storoslop-owned page in project-shared Chromium and applies stealth patches.
...
Reusing one tab name across browser kinds is rejected until the existing tab is closed. Closing storoslop-owned headless pages and owned cmux surfaces closes them. Connected and relay pages remain open. Spawned browser processes remain open unless `kill: true` releases their last managed tab and terminates the process.
