...
- Filesystem
  ...
  - Session files are named `<ISO-timestamp-with-:-and-.-replaced>_<uuidv7>.jsonl` in the session directory; default directory selection is `~/.storoslop/agent/sessions/<encoded-cwd>/` when no override is passed.
