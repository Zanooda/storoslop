...
User plugin state lives under the plugins data root (`~/.storoslop/plugins` by default). On Linux and macOS, `storoslop config init-xdg` initializes the XDG data, state, and cache roots but does not move existing data; with the XDG variables set, initialized roots store new user plugin state under `$XDG_DATA_HOME/storoslop/plugins`:
