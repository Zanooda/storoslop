...
## Native `.storoslop` files

The native provider is the recommended format for new projects. It reads from your user agent directory and from `.storoslop/` directories inside a project, and it has the highest discovery priority, so its files win over every other convention at the same scope.

| File                                          | Scope   | Behavior                                                                                                                                                                                                                                             |
| --------------------------------------------- | ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `~/.storoslop/agent/AGENTS.md`                      | User    | User-level context for every session unless the `native` provider is disabled.                                                                                                                                                                       |
| `<nearest-non-empty-ancestor>/.storoslop/AGENTS.md` | Project | Project context, but only when `AGENTS.md` exists in the **nearest non-empty `.storoslop/` directory** found while walking from cwd toward the repository root. storoslop does not continue to a farther `.storoslop/` directory when the nearest one lacks this file. |
| `~/.storoslop/agent/RULES.md`                       | User    | User-level sticky rule content. Loaded as an always-apply rule, not as a context file.                                                                                                                                                               |
| `<nearest-non-empty-ancestor>/.storoslop/RULES.md`  | Project | Project sticky content, but only when `RULES.md` exists in the same nearest non-empty `.storoslop/` directory selected by the walk.                                                                                                                        |
...
- The native context file is `repo/packages/api/.storoslop/AGENTS.md` (the nearest one). `repo/.storoslop/AGENTS.md` is **not** also included.
- Because `repo/packages/api/.storoslop/` is the nearest non-empty native directory, project sticky content can only come from `repo/packages/api/.storoslop/RULES.md`. If that file is absent, `repo/.storoslop/RULES.md` is **not** used.
...
| ----------- | ------------------------------------------- | -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `native`    | `.storoslop/AGENTS.md`                            | User + project | Recommended storoslop format. User file in the active native agent directory; project file is read only from the nearest non-empty `.storoslop/` directory walking toward the repo root.                                                                                                                                                                                 |
...
- **One user context file** is kept across all providers. Because `native` has the highest priority, `~/.storoslop/agent/AGENTS.md` shadows every other user-level context file.
...
- If you add `repo/packages/api/.storoslop/AGENTS.md`, `native` (priority 100) wins depth 0 outright, shadowing both lower-priority files.
...
- It is read **only** at native locations: the active user agent directory and the nearest non-empty project `.storoslop/` directory selected by the cwd-to-repository-root walk. If that project directory has no `RULES.md`, storoslop does not fall back to a farther `.storoslop/RULES.md`.
...
- It is re-discovered from disk when a session starts and on session-scoped rebuilds such as `/clear` and `/new`, so creating or editing it while storoslop is running takes effect at the next reset — no restart required.
...
Turn a provider off with the `disabledProviders` setting in `~/.storoslop/agent/config.yml`, a project's `.storoslop/config.yml`, or a `--config` overlay:

```yaml
# .storoslop/config.yml
disabledProviders:
  - claude
  - github
```
...
```yaml
# ~/.storoslop/agent/config.yml, .storoslop/config.yml, or a --config overlay
disabledExtensions:
...
- Native project context is read only from the nearest non-empty `.storoslop/` directory. That directory must contain a non-empty `AGENTS.md`; if it does not, discovery does not continue to a farther native directory.
- A standalone `AGENTS.md` is handled by `agents-md`, not `native`; a standalone `CLAUDE.md` is handled by `claude-md`, not `native`.
...
At one user scope or project depth, the higher-priority provider shadows the others (native > claude > agents/codex > gemini > opencode > github > agents-md > claude-md). To force deterministic behavior, move your guidance into `.storoslop/AGENTS.md` (native always wins) or disable the competing discovery provider.

### User context disappeared

Only one user-level context file survives, and `~/.storoslop/agent/AGENTS.md` has the highest priority. If it exists, it shadows user-level `~/.claude/CLAUDE.md`, `~/.codex/AGENTS.md`, `~/.gemini/GEMINI.md`, `~/.config/opencode/AGENTS.md`, `~/.copilot/copilot-instructions.md`, and `~/.agent`/`~/.agents` files. Consolidate user guidance into the native file or remove the native one if you prefer another tool's file.

### A `RULES.md` file is ignored

Only native `RULES.md` locations are sticky: the active user agent directory and the nearest non-empty project `.storoslop/` directory selected from cwd toward the repo root. If a nearer non-empty `.storoslop/` directory exists, it blocks farther native directories even when it has no `RULES.md`. A `RULES.md` anywhere else is not a recognized convention.
