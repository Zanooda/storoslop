...
Providers are the model backends `storoslop` can route requests to: Anthropic, OpenAI, Google Gemini, Groq, OpenRouter, Mistral, xAI, local engines like Ollama, hosted gateways, custom `models.yml` providers, and providers registered by extensions.
...
## How `storoslop` decides a provider is available
...
2. Custom provider and model entries from `~/.storoslop/agent/models.yml`.
...
When a provider needs an API key, `storoslop` resolves it in this order (first match wins):
...
Stored credentials live in the auth store at `~/.storoslop/agent/agent.db` for local auth, or in the configured auth-broker snapshot when running in broker mode. (`PI_CODING_AGENT_DIR` relocates the `~/.storoslop/agent` base, and the auth store moves with it.)
...
Outside a session, `storoslop login [<provider>]` runs the same login from the terminal: it prints the auth URL (and opens it in your browser), reads any prompts from stdin, and saves to the same store sessions use — local `agent.db`, or the configured auth broker. Without a provider it shows a numbered picker.

For headless or remote setups backed by a shared auth broker, the CLI exposes `storoslop auth-broker login <provider>` / `storoslop auth-broker logout` (and `status`, `list`, `import`, `migrate`). See [Secrets and credentials](./secrets.md) for the broker model.

When a model has no credentials, `storoslop` tells you to run `/login` or set the provider's environment variable.
...
`storoslop` eagerly loads `.env` files into the process environment before any provider lookup. It reads four files and, for each variable, the **first** source that defines it wins. Effective precedence, high to low:

1. The process environment inherited by `storoslop` (already-set variables always win).
2. `<cwd>/.env`
3. `~/.storoslop/agent/.env`
4. `~/.storoslop/.env`
5. `~/.env`

A variable already present in the process environment is never overwritten by a `.env` file. Among the files, a value set in `<cwd>/.env` wins over `~/.storoslop/agent/.env`, which wins over `~/.storoslop/.env`, which wins over `~/.env`. So a shell-exported `OPENAI_API_KEY` beats every `.env` file, and a project's `<cwd>/.env` beats your home `~/.env`.
...
```yaml
# ~/.storoslop/agent/config.yml or <project>/.storoslop/config.yml
disabledProviders:
  - anthropic
  - openai
  - google
...
Project settings live in `<project>/.storoslop/config.yml`. Use them when one repository must allow or hide a different provider set than your global default:

```yaml
# <project>/.storoslop/config.yml
disabledProviders:
  - openai
  - openrouter
```
...
Custom providers live in `~/.storoslop/agent/models.yml` under `providers:`. A provider ID defined there participates in the same selection, credential resolution, and `disabledProviders` rules as built-in providers.
...
**A provider still appears even though I disabled it.** `disabledProviders` arrays are replaced, not merged: a project `<project>/.storoslop/config.yml` array fully overrides the global one. Verify the _effective_ list for the directory you are in (path-scoped entries only apply at or under their configured path), and confirm the ID is spelled exactly. Use `storoslop config get disabledProviders` to inspect the merged value (see [Settings](./settings.md)).
