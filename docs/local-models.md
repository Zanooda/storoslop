...
- **Non-FHS distros (NixOS, and any host without `libstdc++.so.6` on the loader path)**: the
  ...
  the storoslop executable's own RPATH can resolve them. Set `storoslop_NATIVE_LIBRARY_PATH` to the
  colon-separated directories holding those libraries; storoslop appends it to `LD_LIBRARY_PATH` for the
...
- **One worker per model, keep-alive not persistent**: every local model is served by exactly one
  worker process on the machine that owns the socket `~/.storoslop/run/tiny/<model>-<backend>.sock`
  (Windows: a named pipe). The first storoslop process that needs the model spawns the worker detached
  (log next to the socket, `*.sock.log`); every other storoslop process just connects, so the model is
  resident once rather than once per instance. Nothing supervises it: the worker exits on its own
  after 15 minutes without a request (`storoslop_TINY_WORKER_IDLE_MS` overrides the window for tests),
  unlinks its socket, and the next request from any storoslop process spawns a fresh one. Concurrent
  ...
  `mlx|<mlx-lm version>|<script crc>`), so a storoslop upgrade or a changed
...
- **MLX backend (Apple silicon)**: `PI_TINY_DEVICE=mlx` (or `metal`) swaps the worker itself, not
  ...
  that storoslop installs under `~/.storoslop/agent/cache/tiny-mlx-runtime/` on first use (via `uv`, else
  `python3 -m venv` with Python ≥ 3.10). It downloads the model's pre-quantized 4-bit MLX export
  (`mlxRepo` in the registry) into `~/.storoslop/agent/cache/tiny-models/mlx/` with per-byte progress,
  ...
  install error, non-Apple host) storoslop logs a warning and uses the ONNX CPU worker for the rest of
  the process. Measured on an M4 Max: cold venv install + LFM2.5-230M download + load 15.7s; a
  second storoslop instance attaches to a running worker in well under a second; titles 15–60ms after
...
When `modelRoles.tiny` is unset, title generation resolves its built-in online role path; no local weights are downloaded automatically. The default download for a bare `storoslop tiny-models` command is `lfm2.5-230m`.
