...
Settings are stored as plain YAML mappings. Every key, its type, default, and enum values come from its setting definition (declared with `register(...)` next to the owning feature, e.g. `packages/coding-agent/src/tools/settings.ts`). `storoslop config` exposes the complete schema; the interactive `/settings` panel exposes the schema entries that have UI metadata.
...
- For instruction files discovered into the agent context (`AGENTS.md`, `.storoslop/`, etc.), see [Context files](./context-files.md).
...
| `storoslop config set <key> <value>` | Parse `<value>` against the key's schema type, write it to the global main YAML file, and print the value written. When another source still supplies the effective value, it says which instead (`--json`: `overriddenBy` is the env var name, or `project`, `overlay`, or `runtime`; `fallbackEnv` names a fallback env var used while the saved value is blank). |
| `storoslop config reset <key>`       | Delete the key from the global config, so any other layer that configures it (such as project config) or else the schema default applies, and later default changes still reach you. Prints the resulting effective value.                                                                        |
| `storoslop config path`              | Print the active agent directory (honors `PI_CODING_AGENT_DIR`).                                                                                                                                                                                                                                  |
| `storoslop config init-xdg`          | On Linux and macOS, create the `storoslop` directories under the effective XDG data, state, and cache homes. It does not move existing files or set the XDG environment variables. Other platforms exit non-zero.                                                                                       |
...
4. **Project settings** — `<cwd>/.storoslop/settings.json` then `<cwd>/.storoslop/config.yml` (and contributions from other discovery providers at project level).
5. **Global settings** — `~/.storoslop/agent/config.yml`.
...
Create `<repo>/.storoslop/config.yml` when a repository needs its own settings:
...
# <repo>/.storoslop/config.yml — inside this repo ONLY groq is disabled
disabledProviders:
  - groq
```
...
| `autolearn.enabled`           | boolean | `false`       | Experimental: after the agent stops, nudge it to capture lessons to memory and create/enhance isolated managed skills under `~/.storoslop/agent/managed-skills`. Enables the `manage_skill` tool (and `learn` when a memory backend is active). |
...
Every schema path not individually tabulated in this catalog is explicitly deferred to `storoslop config list`. Additional groups include:
...
When neither `~/.storoslop/agent/config.yml` nor the compatible `config.yaml` exists, startup builds canonical `config.yml` once from legacy sources, then writes the result:

1. `~/.storoslop/agent/settings.json` (renamed to `settings.json.bak` after a successful parse).
