...
3. **Project settings** — `<cwd>/.storoslop/settings.json` then `<cwd>/.storoslop/config.yml` (and contributions from other discovery providers at project level).
4. **Global settings** — `~/.storoslop/agent/config.yml`.
