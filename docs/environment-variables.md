...
3. Active agent `.env` (normally `~/.storoslop/agent/.env`) for keys whose current value is empty/unset
4. Active config-root `.env` (normally `~/.storoslop/.env`) for keys whose current value is empty/unset
5. Home `.env` (`~/.env`) for keys whose current value is empty/unset

The agent/root locations respect profiles, `PI_CONFIG_DIR`, and—only for the default profile—`PI_CODING_AGENT_DIR`. Dotenv names must be shell identifiers (`[A-Za-z_][A-Za-z0-9_]*`); unsafe names/values are discarded. storoslop's parser keeps values literal; only Bun's own launch-directory dotenv autoload may perform Bun-supported expansion before this module runs.
...
| `OMP_AUTH_BROKER_SNAPSHOT_CACHE`    | Path to the encrypted local broker snapshot cache                                            | Optional in broker mode                                                                                                   | Defaults to `~/.storoslop/cache/auth-broker-snapshot.enc` (or XDG cache equivalent). Useful for tests, ephemeral hosts, or relocating the `0600` cache file.                                                                                                                               |
...
The gateway has no dedicated env vars — it inherits `OMP_AUTH_BROKER_*`. Its own inbound bearer token lives at `<config-dir>/auth-gateway.token` and is managed via `storoslop auth-gateway token`.
...
| `PI_OPENAI_STREAM_FIRST_EVENT_TIMEOUT_MS`   | OpenAI-specific first-event timeout override; `0` disables and takes precedence over the generic value. `storoslop config set providers.streamFirstEventTimeoutSeconds <seconds>` provides the persisted equivalent |
| `PI_OPENAI_STREAM_IDLE_TIMEOUT_MS`          | OpenAI-specific idle timeout override; `0` disables and takes precedence over the generic value. `storoslop config set providers.streamIdleTimeoutSeconds <seconds>` provides the persisted equivalent              |
...
SearXNG also reads the equivalent `searxng.endpoint`, `searxng.token`, `searxng.basicUsername`, and `searxng.basicPassword` settings from `~/.storoslop/agent/config.yml`; environment variables are fallbacks.
...
| `OLLAMA_CONTEXT_LENGTH`      | Positive integer context-window override for implicit Ollama discovery; affects storoslop context budgeting only and does not change Ollama's runtime `num_ctx`                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| `PI_CONFIG_DIR`                                     | Config root dirname under home (default `.storoslop`)                                                                            |
...
| `XDG_DATA_HOME`, `XDG_STATE_HOME`, `XDG_CACHE_HOME` | On macOS/Linux, redirect corresponding storoslop paths only when the target `omp` root (or named-profile root) already exists    |
...
storoslop initializes OTLP export only when at least one signal has an endpoint. `OTEL_SDK_DISABLED=true` disables initialization.
...
| `OTEL_LOG_LEVEL`                                                                                                | Minimum exported storoslop log level                                                                  |
