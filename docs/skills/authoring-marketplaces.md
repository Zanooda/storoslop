...
```json
{
  ...
  "metadata": {
    "description": "Official Acme plugins for storoslop"
  },
  "plugins": [
    {
      "name": "acme-linter",
...
- **user** (default) — installed in the user plugins data root's `installed_plugins.json` (`~/.storoslop/plugins/installed_plugins.json` by default), available in all projects. On Linux and macOS, `storoslop config init-xdg` initializes (but does not migrate data into) the XDG roots; with the XDG variables set, initialized roots store new user state in `$XDG_DATA_HOME/storoslop/plugins/installed_plugins.json`.
