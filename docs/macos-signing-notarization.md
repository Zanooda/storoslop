...
- `curl -fsSL https://raw.githubusercontent.com/Zanooda/storoslop/main/scripts/install.sh | sh` — `curl` sets no quarantine bit, so
...
gh secret list --repo Zanooda/storoslop                       # confirm
```
...
APPLE_API_KEY_ID=… APPLE_API_ISSUER_ID=… APPLE_API_KEY=… \
  bash scripts/ci-macos-sign.sh packages/coding-agent/binaries/storoslop-darwin-arm64
```
