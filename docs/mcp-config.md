# MCP configuration in storoslop

This guide explains how to add, edit, and validate MCP servers for the storoslop coding agent.
...
storoslop can discover MCP servers from multiple tools (`.claude/`, `.cursor/`, `.vscode/`, `opencode.json`, and more), but for storoslop-native configuration you should usually use one of these primary files:

- Project: `.storoslop/mcp.json`
- User: `~/.storoslop/agent/mcp.json` (or `~/.storoslop/profiles/<name>/agent/mcp.json` when a named profile is active — see [Profiles](#profiles))

The native provider also reads `.storoslop/.mcp.json` and `~/.storoslop/agent/.mcp.json` for compatibility, but storoslop writes to the primary `mcp.json` paths above.

storoslop also accepts fallback standalone files in the project root:

- `mcp.json`
- `.mcp.json`

Use `.storoslop/mcp.json` or `~/.storoslop/agent/mcp.json` when you want storoslop to own the configuration. Use root `mcp.json` / `.mcp.json` only when you want a portable fallback file that other MCP clients may also read.

### Imported tool configs

storoslop also translates these current tool-native sources:
...
- installed Claude marketplace plugins and storoslop extension packages that declare MCP servers

For Claude Code, Codex, Gemini CLI, Cursor, and Windsurf, the project entry is encountered before its same-named user entry — matching storoslop-native config, whose project entry precedes its active-profile user entry — so a project `enabled: false` suppresses a same-named user server. OpenCode currently encounters the user entry first. Cross-provider priority is listed in [Discovery and precedence](#discovery-and-precedence).
...
MCP follows the same profile rules as the rest of storoslop-native config; see [Configuration Discovery → Profiles](./config-usage.md#profiles).
...
```json
{
  "$schema": "https://raw.githubusercontent.com/Zanooda/storoslop/main/packages/coding-agent/src/config/mcp-schema.json",
  "mcpServers": {}
}
```

storoslop now writes this automatically when `/mcp add`, `/mcp enable`, `/mcp disable`, `/mcp reauth`, or other config-writing flows create or update an storoslop-managed MCP file.

## File shape

storoslop supports this top-level structure:
...
The config writer accepts names up to 100 characters containing letters, numbers, `_`, `-`, `.`, and `:`. The bundled schema currently omits `:` from its name pattern, so an storoslop-managed namespaced plugin entry such as `cloudflare:cloudflare-api` may be valid at runtime while an editor reports a schema error.
...
- `requestIdFormat?: "number" | "string"` — outgoing JSON-RPC request-id encoding; defaults to per-transport integers. `"string"` uses collision-resistant snowflake IDs. This storoslop-specific field is read only from storoslop-native files, root `mcp.json` / `.mcp.json`, and storoslop extension packages; configs translated from other tools ignore it.
- `instructions?: boolean` — include server-provided instructions in the system prompt (default: `true`). Set `false` to omit that server's instructions without disabling its tools. Like `requestIdFormat`, this is storoslop-specific and is ignored in configs translated from other tools.
...
`OMP_MCP_TIMEOUT_MS` has process-wide precedence over every per-server `timeout`. Set it to `0` to disable client-side timeouts, or to a positive millisecond value such as `120000`. If it is unset or invalid, storoslop uses the server value and then the 30-second default; invalid values are logged and ignored.
...
storoslop understands two auth-related objects.
...
For managed OAuth, `auth` tells storoslop how to find and refresh a stored credential. Although `"apikey"` is an accepted `type`, it does not load or inject an API key from auth storage. Put API keys directly in stdio `env` or remote `headers` (prefer an environment-variable or `!command` indirection described below).

You normally do not need to write this block: when storoslop completes an OAuth flow for an `http`/`sse` server, it stores the credential under a deterministic id derived from the active profile and server URL (`mcp_oauth:profile:<profile>:<url>`), with the refresh material embedded. Any
...
still honored when it resolves; if it points at another profile's row, storoslop falls
...
`prompt` controls the OAuth `prompt` authorization parameter. By default storoslop omits it, except that a requested `offline_access` scope defaults to `"consent"` so the provider can issue refresh access. Set it explicitly to a provider-supported value such as `"consent"` or `"select_account"`, or to `""` to force omission.
...
storoslop expands `${VAR}` and `${VAR:-default}` placeholders while discovering MCP configs from storoslop-native files and standalone fallback files. Expansion applies recursively to string values in `command`, `args`, `env`, `cwd`, `url`, `headers`, `auth`, and `oauth`; unresolved placeholders remain literal strings.
...
Before storoslop launches a stdio server or makes an HTTP/SSE request, it resolves stdio `env` values and HTTP/SSE `headers` values like this:

1. If a value starts with `!`, storoslop runs the rest as a shell command with a 10s timeout and uses trimmed stdout. Successful results are cached for the lifetime of the process.
2. If the command fails, times out, or prints only whitespace, that `env`/`headers` entry is omitted.
3. Otherwise storoslop checks whether the whole value names an environment variable.
4. If that environment variable is set to a non-empty value, storoslop uses the environment value; otherwise it uses the string literally.
...
`/mcp enable` and `/mcp disable` update `enabled` directly when the definition is in an storoslop-owned writable file. storoslop does not mutate another tool's config: for such sources, those commands maintain the user-level allowlist or denylist instead and remove a conflicting stale override.
...
## Validation rules storoslop enforces
...
- If you paste a remote server config and forget `"type": "http"`, storoslop will treat it as `stdio` and complain that `command` is missing
- `sse` remains valid for compatibility, but new hosted servers should usually be configured as `http`

## Discovery and precedence

storoslop loads providers in descending priority. The MCP-capable order is:

1. storoslop native config
2. storoslop extension packages
3. Claude Code
...
Within storoslop native config, project `.storoslop/mcp.json` precedes `.storoslop/.mcp.json`, then the active profile's user `mcp.json` and `.mcp.json`. Root fallback `mcp.json` precedes root `.mcp.json`. In practice:

- prefer `.storoslop/mcp.json` or the active profile's user `mcp.json` for an storoslop-specific override
...
Pick one transport. storoslop treats `command` as stdio and `url` as http/sse.
...
### The server exists in another tool's config but not in storoslop

Run `/mcp list`. storoslop discovers many third-party MCP files, but project-level loading can also be disabled via the `mcp.enableProjectConfig` setting, and a user-level `disabledServers` entry can suppress a server by name.
...
Malformed JSON or a missing/invalid server map makes that provider contribute no entries from the file; depending on the provider, storoslop records a discovery warning or logs the parse failure rather than failing the session. Correct the JSON shape, then run `/mcp reload` and `/mcp list`.
