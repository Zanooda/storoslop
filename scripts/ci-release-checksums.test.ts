...
describe("formatChecksums", () => {
	it("sorts entries by basename and formats as `sha256sum -c`-compatible lines", () => {
		const output = formatChecksums([
			{ name: "storoslop-linux-x64", sha256: "b".repeat(64) },
			{ name: "storoslop-darwin-arm64", sha256: "a".repeat(64) },
		]);
		expect(output).toBe(`${"a".repeat(64)}  storoslop-darwin-arm64\n${"b".repeat(64)}  storoslop-linux-x64\n`);
	});
